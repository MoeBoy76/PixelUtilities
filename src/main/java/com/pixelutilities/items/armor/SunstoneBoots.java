...
public class SunstoneBoots extends ItemArmor
{
	private boolean isEquipped = false;

	public SunstoneBoots(ArmorMaterial par2EnumArmorMaterial, int par3, int par4) {
	...
	public void registerIcons(IIconRegister par1IconRegister) {
		this.itemIcon = par1IconRegister.registerIcon("pixelutilities:armor/SunstoneBoots");
	}
		...
		if(player instanceof EntityPlayerMP)
		{
			...
			if(thePlayer.getEquipmentInSlot(1) != null)
			{
				...
				if(thePlayer.getEquipmentInSlot(1) != null && thePlayer.getEquipmentInSlot(2) != null && thePlayer.getEquipmentInSlot(3) != null && thePlayer.getEquipmentInSlot(4) != null)
				{
					...
					if(boots instanceof SunstoneBoots && legs instanceof SunstoneLegs && plate instanceof SunstonePlate && helm instanceof SunstoneHelmet)
					{
						isEquipped = true;
						if(!stack.isItemEnchanted())
						{
							stack.addEnchantment(Enchantment.protection, 4);
							stack.addEnchantment(Enchantment.projectileProtection, 4);
						}
					}
					else if(boots instanceof SunstoneBoots)
					{
						isEquipped = true;
						stack = new ItemStack(Basemod.instance.sunstoneBoots, 1, boots.getDamage(stack));
					}
					else
					{
						isEquipped = false;
